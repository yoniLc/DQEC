...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        shape_inp = x.shape
        x = x.view(-1,shape_inp[2],shape_inp[3])
        for idx, layer in enumerate(self.layers, start=1):
            x = layer(x, mask)
            if idx == len(self.layers)//2 and len(self.layers) > 1:
                x = x.view(shape_inp).mean(1)
                x = self.norm2(x)      
...
class ECC_Transformer(nn.Module):
    ...
    def loss(self, z_pred, z2):
        ...
        loss2 = F.binary_cross_entropy_with_logits(self.magnitude_pred.mean(1), 1-z2)
        self.magnitude_pred = []
        return loss1,loss2
